...
export const timelineDays = derived(timelineIndex, $index => dayTotals($index));

/** First and last day with ink, or null on an empty index. */
export const timelineRangeExtent = derived(timelineIndex, $index => timelineExtent($index));
...
export const timelineRangeDays = derived(
  [timelineIndex, timelineRange, timelineDays],
  ([$index, $range, $days]) => {
...
export async function loadTimeline(options = {}) {
  ...
  try {
    ...
    if (!range.from || !range.to) {
      const extent = timelineExtent(index);
